...
public partial class Problem : ProblemPart<InputRow>
{
    ...
    protected override string PartA(IEnumerable<InputRow> datas)
    {
        ...
        Bounds smallest = Bounds.Zero;
        foreach (var elf in elves)
        {
            if (elf.position.X < smallest.TopLeft.X) smallest = smallest.Clone(topLeft: new Point(elf.position.X, smallest.TopLeft.Y));
            if (elf.position.Y < smallest.TopLeft.Y) smallest = smallest.Clone(topLeft: new Point(smallest.TopLeft.X, elf.position.Y));
            if (elf.position.X > smallest.BottomRight.X) smallest = smallest.Clone(bottomRight: new Point(elf.position.X, smallest.BottomRight.Y));
            if (elf.position.Y > smallest.BottomRight.Y) smallest = smallest.Clone(bottomRight: new Point(smallest.BottomRight.X, elf.position.Y));
        }
        int empties = 0;
        for (y = smallest.TopLeft.Y; y <= smallest.BottomRight.Y; y++)
        {
            for (x = smallest.TopLeft.X; x <= smallest.BottomRight.X; x++)
            {
                if (area[y + zero.Y, x + zero.X].elves == 0) empties++;
            }
        }
